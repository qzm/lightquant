...
class RiskAwareMovingAverageStrategy(BaseStrategy):
    ...
    def initialize(self) -> None:
        ...
        logger.info(f"初始化策略: {self.config.name}, 交易对: {self.symbol}, "
    ...
    def on_candle(self, candle: Candle) -> StrategyResult:
        ...
        if prev_short_ma <= prev_long_ma and self.short_ma > self.long_ma:
            if self.position <= 0:  # 如果没有多头持仓或者有空头持仓
                # 平空仓
                if self.position < 0:
                    result.add_log(f"平空仓信号: 短期MA({self.short_ma:.2f}) 上穿 长期MA({self.long_ma:.2f})")
                
                # 开多仓
                result.add_log(f"买入信号: 短期MA({self.short_ma:.2f}) 上穿 长期MA({self.long_ma:.2f})")
        ...
        # 短期均线下穿长期均线
            ...
            if self.position >= 0:  # 如果没有空头持仓或者有多头持仓
                # 平多仓
                if self.position > 0:
                    result.add_log(f"平多仓信号: 短期MA({self.short_ma:.2f}) 下穿 长期MA({self.long_ma:.2f})")
                
                # 开空仓
                result.add_log(f"卖出信号: 短期MA({self.short_ma:.2f}) 下穿 长期MA({self.long_ma:.2f})")
                order = self.create_market_order(
                    symbol=self.symbol,
                    side=OrderSide.SELL,
                    amount=self.position_size
...
class MockMarketDataService(MarketDataService):
    ...
    def __init__(self, repository: MarketDataRepository):
        ...
        self.mock_candles: Dict[str, Dict[str, List[Candle]]] = {}  # symbol -> timeframe -> candles
    
    def add_mock_candles(self, symbol: str, timeframe: str, candles: List[Candle]) -> None:
        """
        添加模拟K线数据
...
def plot_backtest_results(backtest_results: Dict[str, Any], with_risk: bool = True) -> None:
    """
    绘制回测结果
    ...
    # 设置标题
    strategy_name = "带风险管理的移动平均线策略" if with_risk else "移动平均线策略（无风险管理）"
...
def run_backtest(with_risk: bool = True) -> Dict[str, Any]:
    ...
    config = StrategyConfig(
        name="带风险管理的移动平均线策略" if with_risk else "移动平均线策略（无风险管理）",
        symbols=[symbol],
    ...
    strategy_id = backtest_engine.create_strategy(
        strategy_class=RiskAwareMovingAverageStrategy,
        config=config
    )
    ...
    backtest_results['trades_per_day'] = [(datetime.fromisoformat(date), count) 
...
def main():
    ...
    if with_risk_results and no_risk_results:
        ...
        logger.info("=== 回测结果比较 ===")
        logger.info(f"总收益: 带风险管理={with_risk_metrics['total_return']:.2f}%, 无风险管理={no_risk_metrics['total_return']:.2f}%")
        logger.info(f"最大回撤: 带风险管理={with_risk_metrics['max_drawdown']:.2f}%, 无风险管理={no_risk_metrics['max_drawdown']:.2f}%")
        logger.info(f"夏普比率: 带风险管理={with_risk_metrics['sharpe_ratio']:.2f}, 无风险管理={no_risk_metrics['sharpe_ratio']:.2f}")
        logger.info(f"交易次数: 带风险管理={with_risk_metrics['total_trades']}, 无风险管理={no_risk_metrics['total_trades']}")
