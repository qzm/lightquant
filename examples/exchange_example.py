...
async def main():
    ...
    for i, candle in enumerate(candles):
        print(f"  {i+1}. 时间: {candle.timestamp}, 开: {candle.open}, 高: {candle.high}, 低: {candle.low}, 收: {candle.close}, 量: {candle.volume}")
    ...
    if api_key and api_secret:
        ...
        print(f"订单参数: 交易对={order.symbol}, 类型={order.type}, 方向={order.side}, 数量={order.amount}, 价格={order.price}")
