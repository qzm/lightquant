...
from lightquant.domain.models.market_data import Ticker, Candle, OrderBook, OrderBookEntry
...
def main():
    # 初始化数据库
    connection_string = os.environ.get("DATABASE_URL", "sqlite:///lightquant_example.db")
    ...
    print(f"查询订单: {retrieved_order.id}, 交易对: {retrieved_order.params.symbol}, 价格: {retrieved_order.params.price}")
    ...
    for currency, balance in retrieved_account.balances.items():
        print(f"  {currency}: 可用: {balance.free}, 冻结: {balance.used}, 总计: {balance.total}")
    ...
    print(f"查询策略: {retrieved_strategy.id}, 名称: {retrieved_strategy.config.name}, 状态: {retrieved_strategy.status}")
    ...
    # 订单簿
    bids = [OrderBookEntry(price=19990.0 - i * 10, amount=1.0 + i * 0.1) for i in range(10)]
    asks = [OrderBookEntry(price=20010.0 + i * 10, amount=1.0 + i * 0.1) for i in range(10)]
    ...
    print(f"订单簿已保存: {order_book.symbol}, 买一价: {order_book.bids[0].price}, 卖一价: {order_book.asks[0].price}")
    ...
    print(f"查询行情: {retrieved_ticker.symbol}, 最新价: {retrieved_ticker.last}, 时间: {retrieved_ticker.timestamp}")
    
    # 查询K线
    retrieved_candles = market_data_repo.get_candles("BTC/USDT", "binance", "1h", limit=5)
    print(f"查询K线: {len(retrieved_candles)}根")
    for i, candle in enumerate(retrieved_candles):
        print(f"  {i+1}. 时间: {candle.timestamp}, 开: {candle.open}, 高: {candle.high}, 低: {candle.low}, 收: {candle.close}")
    
    # 查询订单簿
    retrieved_order_book = market_data_repo.get_order_book("BTC/USDT", "binance")
    print(f"查询订单簿: {retrieved_order_book.symbol}, 买一价: {retrieved_order_book.bids[0].price}, 卖一价: {retrieved_order_book.asks[0].price}")
