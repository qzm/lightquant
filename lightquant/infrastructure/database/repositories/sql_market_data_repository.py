...
class SQLMarketDataRepository(MarketDataRepository):
    ...
    def get_ticker(self, symbol: str, exchange_id: str) -> Optional[Ticker]:
        ...
        with self._db_manager.session() as session:
            ticker_model = session.query(TickerModel).filter(
                TickerModel.symbol == symbol,
                TickerModel.exchange_id == exchange_id
            ).order_by(desc(TickerModel.timestamp)).first()
    ...
    def get_tickers(self, exchange_id: str) -> Dict[str, Ticker]:
        ...
        with self._db_manager.session() as session:
            ...
            symbols = session.query(TickerModel.symbol).filter(
                TickerModel.exchange_id == exchange_id
            ).distinct().all()
    ...
    def save_ticker(self, ticker: Ticker) -> None:
        ...
        with self._db_manager.session() as session:
            ticker_model = TickerModel(
                id=str(ticker.timestamp.timestamp()) + "_" + ticker.symbol + "_" + ticker.exchange_id,
                ...
                quote_volume=ticker.quote_volume if hasattr(ticker, 'quote_volume') else None,
    ...
    def get_candles(
        ...
        with self._db_manager.session() as session:
            ...
            candle_models = query.order_by(desc(CandleModel.timestamp)).limit(limit).all()
        ...
        with self._db_manager.session() as session:
            for candle in candles:
                candle_model = CandleModel(
                    id=str(candle.timestamp.timestamp()) + "_" + candle.symbol + "_" + candle.exchange_id + "_" + candle.timeframe,
                    ...
                    quote_volume=candle.quote_volume if hasattr(candle, 'quote_volume') else None,
                    created_at=datetime.utcnow()
                )
    ...
    def get_order_book(
        ...
        """获取订单簿"""
        with self._db_manager.session() as session:
            order_book_model = session.query(OrderBookModel).filter(
                OrderBookModel.symbol == symbol,
                OrderBookModel.exchange_id == exchange_id
            ).order_by(desc(OrderBookModel.timestamp)).first()
    ...
    def save_order_book(self, order_book: OrderBook) -> None:
        ...
        with self._db_manager.session() as session:
            # 将买单和卖单转换为JSON格式
            bids_json = json.dumps([{"price": bid.price, "amount": bid.amount} for bid in order_book.bids])
            asks_json = json.dumps([{"price": ask.price, "amount": ask.amount} for ask in order_book.asks])
            
            order_book_model = OrderBookModel(
                id=str(order_book.timestamp.timestamp()) + "_" + order_book.symbol + "_" + order_book.exchange_id,
    ...
    def _order_book_to_domain_entity(self, model: OrderBookModel, limit: int) -> OrderBook:
        """将数据库模型转换为领域实体"""
        # 解析JSON格式的买单和卖单
        ...
        # 转换为OrderBookEntry对象
        bids = [OrderBookEntry(price=item["price"], amount=item["amount"]) for item in bids_data[:limit]]
        asks = [OrderBookEntry(price=item["price"], amount=item["amount"]) for item in asks_data[:limit]]
