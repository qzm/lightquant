...
class SQLStrategyRepository(StrategyRepository):
    ...
    def save(self, strategy: Strategy) -> None:
        ...
        with self._db_manager.session() as session:
            # 检查策略是否已存在
            strategy_model = session.query(StrategyModel).filter(StrategyModel.id == strategy.id).first()
            ...
            if strategy_model:
                ...
                strategy_model.performance_metrics = json.dumps(strategy.performance_metrics)
    ...
    def find_by_id(self, strategy_id: str) -> Optional[Strategy]:
        """根据ID查找策略"""
        with self._db_manager.session() as session:
            strategy_model = session.query(StrategyModel).filter(StrategyModel.id == strategy_id).first()
    ...
    def find_by_status(self, status: StrategyStatus) -> List[Strategy]:
        """根据状态查找策略"""
        with self._db_manager.session() as session:
            strategy_models = session.query(StrategyModel).filter(
                StrategyModel.status == self._map_strategy_status(status)
            ).all()
            return [self._to_domain_entity(model, session) for model in strategy_models]
    ...
    def find_by_exchange_id(self, exchange_id: str) -> List[Strategy]:
        ...
        with self._db_manager.session() as session:
            ...
            strategy_models = session.query(StrategyModel).filter(
                StrategyModel.exchange_ids.like(f'%"{exchange_id}"%')
            ).all()
    ...
    def find_by_symbol(self, symbol: str) -> List[Strategy]:
        ...
        with self._db_manager.session() as session:
            ...
            # 这里使用简单的模糊匹配，实际使用时可能需要更精确的查询
            strategy_models = session.query(StrategyModel).filter(
                StrategyModel.symbols.like(f'%"{symbol}"%')
            ).all()
    ...
    def _to_domain_entity(self, model: StrategyModel, session: Session) -> Strategy:
        ...
        strategy._performance_metrics = json.loads(model.performance_metrics) if model.performance_metrics else {}
