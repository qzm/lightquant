...
class BinanceAdapter(ExchangeAdapter):
    ...
    async def fetch_candles(
        ...
        try:
            ...
            candles_data = await self._exchange.fetch_ohlcv(symbol, timeframe, since_timestamp, limit)
    ...
    async def fetch_order(self, order: Order) -> Tuple[bool, Optional[OrderStatus], Optional[float], Optional[float], Optional[str]]:
        """获取订单状态"""
        try:
            if not order.exchange_order_id:
    ...
    async def fetch_closed_orders(
        ...
        try:
            since_timestamp = int(since.timestamp() * 1000) if since else None
            orders_data = await self._exchange.fetch_closed_orders(symbol, since_timestamp, limit)
            return orders_data
        except Exception as e:
            print(f"获取已完成订单失败: {e}")
            return []
    ...
    def _convert_to_order_book(self, order_book_data: Dict[str, Any], symbol: str) -> OrderBook:
    ...
    def _convert_to_candle(self, candle_data: List[Any], symbol: str, timeframe: str) -> Candle:
