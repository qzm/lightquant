...
class Account(AggregateRoot):
    ...
    def deduct_balance(
        self, asset: str, amount: float, from_locked: bool = False
    ) -> bool:
        """扣除余额（如成交时）"""
        balance = self.get_balance(asset)
        if not balance:
    ...
    def get_equity(
        self, quote_asset: str = "USDT", prices: Dict[str, float] = None
    ) -> float:
        """
        计算账户权益（以指定计价货币表示）
        ...
        for asset, balance in self.balances.items():
            ...
            # 尝试在嵌套字典中查找
                ...
                symbol in prices
                and isinstance(prices[symbol], dict)
                and "last" in prices[symbol]
            ):
