...
class BacktestEngine:
    ...
    def create_strategy(
        ...
        try:
            ...
            # 创建回测账户
            self.account = self._create_backtest_account(config.exchange_ids[0] if config.exchange_ids else "backtest")
    ...
    def _process_order(self, order: Order, candle: Candle) -> None:
        ...
        # 只处理未成交的订单
        if order.status != OrderStatus.OPEN and order.status != OrderStatus.PARTIALLY_FILLED:
            return
        ...
        if order.side == OrderSide.BUY:
            ...
            if quote_currency in account.balances:
                account.balances[quote_currency].free -= order.amount * execution_price + fee
                account.balances[quote_currency].total -= order.amount * execution_price + fee
        ...
        logger.info(f"执行订单: {order.id}, 价格: {execution_price}, 数量: {order.amount}, 手续费: {fee}")
    ...
    def run_backtest(
        ...
        # 更新策略性能指标
        self.strategy_service.update_strategy_performance(strategy_id, performance_metrics)
        
        # 设置回测状态
        self.is_running = False
        
        logger.info(f"回测完成: 策略ID={strategy_id}, 开始时间={start_time}, 结束时间={end_time}")
    ...
    def _calculate_performance_metrics(self) -> Dict[str, Any]:
        ...
        if len(equity_values) > 1:
            ...
            # 计算夏普比率（假设无风险利率为0）
            if std_return > 0:
                sharpe_ratio = (avg_return / std_return) * (252 ** 0.5)  # 假设一年252个交易日
            else:
                sharpe_ratio = 0.0
        else:
            sharpe_ratio = 0.0
        ...
        # 计算平均盈亏比
        avg_profit = sum(order.realized_pnl for order in self.orders if order.realized_pnl > 0) / winning_trades if winning_trades > 0 else 0.0
        avg_loss = sum(abs(order.realized_pnl) for order in self.orders if order.realized_pnl < 0) / losing_trades if losing_trades > 0 else 0.0
